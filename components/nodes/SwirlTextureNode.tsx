...
import { swirlTexture, type SerializedTextureData } from "@/lib/texture"
...
type SwirlTextureNodeData = {
    texture?: SerializedTextureData | null
    error?: string | null
    fallbackValue?: number
}

export const SwirlTextureNode = memo(({ id }: Props) => {
    ...
    const nodeData = (node?.data as SwirlTextureNodeData | undefined) ?? {}
    const texture = nodeData.texture ?? null
    const error = nodeData.error ?? null
    const fallbackValue = nodeData.fallbackValue ?? 0
    const swirlValues = React.useMemo(() => {
    ...
    React.useEffect(() => {
        ...
        try {
            setNode(id, {
                texture: swirlTexture(inputTexture, swirlValues),
                error: null,
            })
        } catch (swirlError) {
            setNode(id, {
                texture: null,
                error:
                    swirlError instanceof Error
                        ? swirlError.message
                        : "Could not swirl the texture.",
            })
        }
    }, [id, inputTexture, setNode, swirlValues])
...
SwirlTextureNode.displayName = "SwirlTextureNode"