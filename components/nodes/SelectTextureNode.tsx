...
import { selectTextureFrames, type SerializedTextureData } from "@/lib/texture";
...
type SelectTextureNodeData = {
    texture?: SerializedTextureData | null;
    error?: string | null;
    fallbackIndex?: number;
}

export const SelectTextureNode = memo(({ id }: Props) => {
    ...
    const nodeData = (node?.data as SelectTextureNodeData | undefined) ?? {};
    const texture = nodeData.texture ?? null;
    const error = nodeData.error ?? null;
    const fallbackIndex = nodeData.fallbackIndex ?? 0;
    const hasValueInput = React.useMemo(() => {
        return edges.some((edge) => edge.target === id && edge.targetHandle === "inputIndex");
    }, [edges, id]);
    const frameIndices = React.useMemo(() => {
        return valueInput?.data ?? createConstantValueFrames(fallbackIndex);
    }, [fallbackIndex, valueInput?.data]);
    const valueFrames = frameIndices.length;
    ...
    React.useEffect(() => {
        ...
        try {
            const selectedTexture = selectTextureFrames(inputTexture, frameIndices);
            setNode(id, { texture: selectedTexture, error: null });
        } catch (selectionError) {
            const message = selectionError instanceof Error
                ? selectionError.message
                : "Could not select texture frames.";

            setNode(id, { texture: null, error: message });
        }
    }, [frameIndices, id, inputTexture, setNode, valueFrames]);
                ...
                <div className="flex flex-col gap-4">
                    ...
                    {hasValueInput ? (
                        <div className="relative text-xs text-secondary-foreground">
                            <Handle type="target" position={Position.Left} id="inputIndex" className="size-3! -left-3! bg-orange-500! border-orange-300!" data-type="value" />
                            Index
                        </div>
                    ) : (
                        <div className="relative">
                            <Handle type="target" position={Position.Left} id="inputIndex" className="size-3! -left-3! top-1/2! -translate-y-1/2 bg-orange-500! border-orange-300!" data-type="value" />
                            <ValueFallbackSlider
                                label="Index"
                                value={fallbackIndex}
                                min={0}
                                max={59}
                                step={1}
                                onChange={(value) => setNode(id, { fallbackIndex: value })}
                            />
                        </div>
                    )}
                </div>
...
SelectTextureNode.displayName = "SelectTextureNode";