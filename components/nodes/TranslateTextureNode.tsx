...
import { translateTexture, type SerializedTextureData } from "@/lib/texture";
...
type TranslateTextureNodeData = {
    texture?: SerializedTextureData | null;
    error?: string | null;
}

export const TranslateTextureNode = memo(({ id }: Props) => {
    ...
    const xValues = xInput?.data ?? ZERO_VALUE_FRAMES;
    const yValues = yInput?.data ?? ZERO_VALUE_FRAMES;
    const nodeData = (node?.data as TranslateTextureNodeData | undefined) ?? {};
    ...
    React.useEffect(() => {
        ...
        try {
            const translated = translateTexture(inputTexture, xValues, yValues);
            setNode(id, { texture: translated, error: null });
        } catch (translateError) {
            const message = translateError instanceof Error
                ? translateError.message
                : "Could not translate the texture.";
...
TranslateTextureNode.displayName = "TranslateTextureNode";