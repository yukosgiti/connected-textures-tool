...
export function adjustContrastTexture(
  ...
  for (let frameIndex = 0; frameIndex < texture.frames; frameIndex += 1) {
    ...
    for (let offset = 0; offset < frameByteLength; offset += 4) {
      ...
      adjustedPixels[pixelIndex] = Math.round(clampUnit(((red - 0.5) * contrastFactor) + 0.5) * 255);
      adjustedPixels[pixelIndex + 1] = Math.round(clampUnit(((green - 0.5) * contrastFactor) + 0.5) * 255);
      adjustedPixels[pixelIndex + 2] = Math.round(clampUnit(((blue - 0.5) * contrastFactor) + 0.5) * 255);
